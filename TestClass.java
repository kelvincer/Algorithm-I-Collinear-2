...
public class TestClass {
    ...
    public TestClass(Point[] points) {
        ...
        segmentsSlopeOrigins.stream().forEach(item -> {

            item.stream().forEach(segmentAndSlope -> {

                if (lineSegmentClassArrayList.isEmpty()) {
                    lineSegmentClassArrayList.add(segmentAndSlope);
                    finalSegments.add(segmentAndSlope.segment);
                    return;
                }
    ...
    private List<LineSegmentClass> findLines(Point[] points, Point origin) {
        ...
        List<LineSegmentClass> lineSegmentList = slopeOriginPoints.stream()
                                                                  .parallel()
                                                                  .filter(p -> p.points.size() >= 4)
                                                                  .map(o -> new LineSegmentClass(
                                                                          new LineSegment(
                                                                                  o.points.get(0),
                                                                                  o.points.get(
                                                                                          o.points.size()
    ...
    public static void main(String[] args) {
        ...
        TestClass collinear = new TestClass(points);
